...
void Quadruped_InPlace(float t, TrajectoryParams *base_params, RobotGeometry *geom)
{
    ...
    // 基础参数
    float step_h = base_params->step_height; 
    float start_z = base_params->start_z;
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        // 生成原地摆线轨迹 (只在Z轴摆动，X始终为0)
        Gen_Cycloid_Trajectory_InPlace(current_phase, swing_ratio, start_z, step_h, &x, &z);
        ...
        if (norm_phase < swing_ratio) 
        {
            ...
            float q1_hip = q1 + current_hip_offset;
            float q2_with_offset = q2 + current_knee_offset;

            target_q1 = q1_hip * thigh_scale * side_signs[i];
            target_q2 = q2_with_offset * shank_scale * side_signs[i];
        ...
        {
            // ============ 支撑相：足端锁定在 start_z，使用固定的站立姿态 ============
            // 不再进行逆运动学计算，直接使用站立姿态的关节角度
            // 这样足端位置完全固定，不会产生前后漂移
            ...
            float q1_hip = stand_q1 + current_hip_offset;
            float q2_with_offset = stand_q2 + current_knee_offset;
        ...
        float tau_vmc_q1 = 0.0f;
        float tau_vmc_q2 = 0.0f;
        vmc_planar_jt_torque(0, 0, 0, 0, start_z * 0.001f, geom->L1 * 0.001f, geom->L2 * 0.001f, &tau_vmc_q1, &tau_vmc_q2);
...
{
    TrajectoryParams p_local = *base_params;
    p_local.step_length = 10.0f; // 强制步长为0
...
/**
 * 输入: t(float), base_params(TrajectoryParams*), geom(RobotGeometry*), spin_dir(float)
 * 输出: 无
 * 作用: 以站立参数为基础执行原地旋转核心步态, spin_dir>0左旋, spin_dir<0右旋
 */
static void Quadruped_Spin_InPlace_Core(float t, TrajectoryParams *base_params, RobotGeometry *geom, float spin_dir)
{
    float period = base_params->period;
    float kd = 15.0f;
    float speed = 0.0f;

    float spin_step_len = fabsf(base_params->step_length);
    if (spin_step_len < 5.0f) spin_step_len = 25.0f;
    ...
    float start_z = base_params->start_z;
    float swing_ratio = base_params->duty_cycle;
    if (swing_ratio < 0.3f || swing_ratio > 0.8f) swing_ratio = 0.55f;
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        float side_dir = (i == 0 || i == 2) ? -spin_dir : spin_dir;
        float x_back = -spin_step_len * 0.5f * side_dir;
        float x_front = spin_step_len * 0.5f * side_dir;
        ...
        {
            dynamic_torque = 3.5f;
            dynamic_kp_thigh = 70.0f;
            dynamic_kp_shank = 80.0f;
        }
        else
        {
            dynamic_torque = 5.5f;
            dynamic_kp_thigh = 60.0f;
            dynamic_kp_shank = 70.0f;
        }
        ...
        float q1_hip = q1 + current_hip_offset;
        float q2_with_offset = q2 + knee_offset;
...
{
    Quadruped_Spin_InPlace_Core(t, base_params, geom, 1.0f);
}
