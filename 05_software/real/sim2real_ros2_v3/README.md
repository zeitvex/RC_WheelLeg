...
该里程碑对应重排主线的 `v1.0.0`，是 1050 分比赛最终版本。训练架构和策略来源见 `v0.6.0`，比赛 Rough 模型首次归档见 `v0.8.0`，导航打点与路线演进见 `v0.8.1`，ROS 2 迁移过程见 `v0.10.0`～`v0.12.0`。

当前 `main` 的 `v1.0.1` 只是在 `v1.0.0` 基础上补充比赛机器人图片和比赛视频，本目录代码仍以 `v1.0.0` 为比赛部署基线。
