...
: Node("sim2real_runtime_node")
{
  ...
  model_switch_transition_s_ = declare_parameter<double>("model_switch_transition_s", 1.5);
  ...
  model_switch_stand_hold_s_ = declare_parameter<double>("model_switch_stand_hold_s", 0.8);
  ...
  model_switch_release_scale_ = static_cast<float>(
    declare_parameter<double>("model_switch_release_scale", 2.0));
  ...
  RCLCPP_INFO(
    get_logger(),
    "Policy inference setup: rough_model=%s, rough_engine=%s, crawl_model=%s, crawl_engine=%s, crawl_backend=%s, prefer_tensorrt=%s",
...
{
  return mode == ModelMode::Rough || crawl_backend_ == CrawlBackend::Rl;
}
  ...
  const std::array<float, 3> & cmd)
{
  ...
  // Keep the balance controller updated for stability monitoring, but use the
  // configured rough default pose as the actual stand target to avoid a second
  // posture jump between "switch stand" and "runtime rough hold".
  ...
  const std::array<float, 3> & cmd)
{
  ...
  const float left_wheel = std::clamp(
    cmd[0] * crawl_ik_wheel_linear_gain_ - cmd[2] * crawl_ik_wheel_yaw_gain_,
    -crawl_ik_max_wheel_speed_, crawl_ik_max_wheel_speed_);
...
bool PolicyRuntimeNode::switchInferenceModel(ModelMode target_mode)
{
  ...
  if (loaded_model_mode_ == target_mode) {
    current_model_mode_ = target_mode;
    active_default_dof_pos_ = target_mode == ModelMode::Rough ? rough_default_dof_pos_ : crawl_default_dof_pos_;
    RCLCPP_INFO(
      get_logger(),
  ...
  const std::string next_model_path = target_mode == ModelMode::Rough ? rough_model_path_ : crawl_model_path_;
  const std::string next_model_engine_path =
    target_mode == ModelMode::Rough ? rough_model_engine_path_ : crawl_model_engine_path_;
...
void PolicyRuntimeNode::onPolicyLoop()
{
  ...
  if (startup_state_ == StartupState::STARTUP_SOFT_HOLD) {
    ...
    if (model_switch_requested && model_switch_state_ == ModelSwitchState::Idle &&
        requested_model_mode != current_model_mode_) {
      ...
      active_switch_transition_s_ = std::clamp(max_dev * 1.2, 1.0, std::max(1.0, model_switch_transition_s_)) *
        std::max(1.0, model_switch_to_stand_transition_scale_);
      ...
      if (hold_elapsed && stand_ready) {
        ...
        if (switchInferenceModel(requested_model_mode)) {
          ...
          active_switch_transition_s_ = std::clamp(
            max_dev * 1.2, 1.0, std::max(1.0, model_switch_transition_s_)) *
            std::max(1.0, model_switch_to_model_transition_scale_);
  ...
  if (safety_enabled_) {
    auto * active_safety_monitor =
      model_switch_state_ != ModelSwitchState::Idle && model_switch_safety_monitor_ ?
