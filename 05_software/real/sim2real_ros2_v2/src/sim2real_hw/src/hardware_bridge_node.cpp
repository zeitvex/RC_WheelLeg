...
void HardwareBridgeNode::onModelStatus(const std_msgs::msg::String::SharedPtr msg)
{
  ...
  const ActiveModelMode next_mode =
    (current_model == "crawl" || current_model == "ik") ? ActiveModelMode::Crawl : ActiveModelMode::Rough;
  if (next_mode == active_model_mode_) {
    return;
  }

  active_model_mode_ = next_mode;
  active_default_dof_pos_ =
    active_model_mode_ == ActiveModelMode::Crawl ? crawl_default_dof_pos_ : rough_default_dof_pos_;
  RCLCPP_INFO(
    get_logger(),
    "Hardware active model reference updated to %s pose.",
    active_model_mode_ == ActiveModelMode::Crawl ? "ik/crawl" : "rough");
}
...
void HardwareBridgeNode::onEstop(const std_msgs::msg::Bool::SharedPtr msg)
{
  ...
  if (estop_triggered_) {
    ...
    logEvent("WARN", "estop_triggered", "Physical E-stop received over /safety/estop.");
  } else {
    RCLCPP_INFO(get_logger(), "Physical E-stop reset.");
    logEvent("INFO", "estop_reset", "Physical E-stop reset via /safety/estop.");
  }
}
