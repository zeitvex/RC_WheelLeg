...
`rc_mjlab/` 保存 16DOF 轮足机器人的训练、MuJoCo、Sim2Sim 和导航工具累积工作树。历史训练架构由 Git Tag 保留，不在目录中复制 `old`、`new` 或 `final` 版本。

当前工作树由多个里程碑累积：训练主体以 `v0.6.0` 的 `best` 比赛架构为基础，`v0.7.0`～`v0.8.1` 依次补充 MuJoCo、Sim2Sim、比赛 Rough ONNX 和导航打点工具。训练过程可能先获得基模，再调整奖励、课程和环境参数继续训练；模型 checkpoint 的变化不等同于软件架构变化。
